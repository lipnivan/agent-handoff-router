...
primary_transport:
  ...
  status: intended primary command/mutation transport; typed ingress is not documented here as merged or deployed until its repository and deployment state confirm it
fallback_transport:
...
components:
  ...
  agent-drive-bridge:
    ...
    repository_state: typed ingress PR #10 is open draft, not merged
    deployment_state: unknown
    evidence:
      - GitHub PR #10 read-only metadata: OPEN draft, head af6e5adf2d3c872001c5e8765b33353ad3e7e851
  ...
  agent-runner:
    ...
    repository_state: PR #10 for sequential drain is merged
...
- `lipi-codex/agent-drive-bridge` PR #10 for typed ChatGPT ingress is open and draft; it is not merged or deployed according to this manifest.
...
- `lipnivan/agent-runner` PR #10 for sequential drain is merged, but production checkout/systemd timer deployment on `.39` is not confirmed here.
...
- `agent:review` is an active review lifecycle state, not a runner pickup label. Runner pickup is only `agent:ready`; after `REQUEST_CHANGES`, continuation should reapply `agent:ready` on the same issue/branch/PR.
...
- Confirm Drive ingress merge and deployment status in the canonical `lipi-codex/agent-drive-bridge` repository docs.
